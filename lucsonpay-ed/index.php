<?php
/*
Plugin Name: EDD - lucsonpay Gateway
Plugin URL: http://easydigitaldownloads.com/extension/lucsonpay-gateway
Description: A lucsonpay gateway for Easy Digital Downloads
Version: 1.0
Author: Rohit Kr Singh
Author URI: https://www.lucsonpay.com/
...
function pw_edd_register_gateway($gateways) {
	$gateways['lucsonpay_gateway'] = array('admin_label' => 'lucsonpay Gateway', 'checkout_label' => __('lucsonpay Gateway', 'pw_edd'));
	return $gateways;	
}
add_filter('edd_payment_gateways', 'pw_edd_register_gateway');

function pw_edd_lucsonpay_gateway_cc_form() {
	// register the action to remove default CC form
	return;
}
add_action('edd_lucsonpay_gateway_cc_form', 'pw_edd_lucsonpay_gateway_cc_form');
...
add_action('edd_gateway_lucsonpay_gateway', 'pw_edd_process_payment');
...
//Return Response
function edd_listen_for_lucsonpay_gateway() {

	if ( isset( $_POST['RESPONSE_CODE'] ) && $_POST['CUST_EMAIL'] != "" ) {
            do_action( 'edd_verify_lucsonpay_gateway' );    
                   
	}
}
add_action( 'init', 'edd_listen_for_lucsonpay_gateway' );



function edd_process_lucsonpay_gateway() {
    global $edd_options;
    $payment = edd_insert_payment($payment);
    $order_id=$payment;
          
		  if($_POST['STATUS']!='Captured'){
		            edd_send_back_to_checkout( '?payment-mode=lucsonpay_gateway' );            
		        }
           
           if($_POST['STATUS']=='Captured')
					{
						$payment_meta   = edd_get_payment_meta( $order_id );
						edd_insert_payment_note( $order_id, sprintf( __( 'Thank you for your order . Your transaction has been successful. lucsonpay Transaction ID: %s', 'edd' ) , $_POST['STATUS'] ) );
						...
						edd_record_gateway_error( __( 'lucsonpay Error', 'edd' ), sprintf( __( 'It seems some issue in server to server communication. Kindly connect with administrator.', 'edd' ), '' ), $payment_id );
						...
						wp_redirect( '?page_id=6&payment-mode=lucsonpay_gateway' );
					}
...
add_action( 'edd_verify_lucsonpay_gateway', 'edd_process_lucsonpay_gateway' );
...
function pw_edd_add_settings($settings) {
	...
	$lucsonpay_gateway_settings = array(
		array(
			'id' => 'lucsonpay_gateway_settings',
			'name' => '<strong>' . __('lucsonpay Gateway Settings', 'pw_edd') . '</strong>',
		...
		array(
			...
			'desc' => __('Transaction URL Provided by Lucsonpay', 'pw_edd'),
			'type' => 'text',
			'size' => 'regular'
		),
		array(
	...
	return array_merge($settings, $lucsonpay_gateway_settings);	
}
